#include "DCPFA_controller.h"
#include <unistd.h>

DCPFA_controller::DCPFA_controller() :
...
void DCPFA_controller::Init(argos::TConfigurationNode &node) {
...
void DCPFA_controller::ControlStep() {
	/*
	ofstream log_output_stream;
	log_output_stream.open("dcpfa_log.txt", ios::app);
...
bool DCPFA_controller::IsHoldingFood() {
		return isHoldingFood;
}

bool DCPFA_controller::IsUsingSiteFidelity() {
		return isUsingSiteFidelity;
}

void DCPFA_controller::DecayAndPrunePheromoneMemory(argos::Real current_time) {
	PheromoneMemory.DecayAndPrune(current_time);
}

std::vector<DecentralizedPheromone> DCPFA_controller::ExportActivePheromones() const {
	return PheromoneMemory.ExportActivePheromones();
}

size_t DCPFA_controller::ReceivePheromones(const std::vector<DecentralizedPheromone>& pheromones) {
	return PheromoneMemory.ReceivePheromones(pheromones);
}

size_t DCPFA_controller::GetPheromoneMemorySize() const {
	return PheromoneMemory.Size();
}

void DCPFA_controller::SetPheromoneMemoryLimit(size_t max_pheromones) {
	PheromoneMemory.SetMaxSize(max_pheromones);
}

void DCPFA_controller::DCPFA() {
...
bool DCPFA_controller::IsInTheNest() {
    
	return ((GetPosition() - LoopFunctions->NestPosition).SquareLength()
		< LoopFunctions->NestRadiusSquared);
	}

void DCPFA_controller::SetLoopFunctions(DCPFA_loop_functions* lf) {
	LoopFunctions = lf;
	...
	/* ss << "DCPFA-"<<GIT_BRANCH<<"-"<<GIT_COMMIT_HASH<<"-"
		<< hostname << '-'
		<< getpid() << '-'
		<< (now->tm_year) << '-'
		<< (now->tm_mon + 1) << '-'
...
void DCPFA_controller::Departing()
{
     //LOG<<"Departing..."<<endl;
...
void DCPFA_controller::Searching() {
 //LOG<<"Searching..."<<endl;
...
void DCPFA_controller::Surveying() {
 //LOG<<"Surveying..."<<endl;
	if (survey_count <= 4) { 
 ...
 *****/
void DCPFA_controller::Returning() {
 //LOG<<"Returning..."<<endl;
	//SetHoldingFood();
...
void DCPFA_controller::SetRandomSearchLocation() {
 ...
 *****/
void DCPFA_controller::SetHoldingFood() {
 ...
 *****/
void DCPFA_controller::SetLocalResourceDensity() {
	...
	PheromoneMemory.AddLocalPheromone(controllerID,
									  SimulationTick(),
 ...
 *****/
void DCPFA_controller::SetFidelityList(argos::CVector2 newFidelity) {
 ...
 *****/
void DCPFA_controller::SetFidelityList() {
	std::vector<argos::CVector2> newFidelityList;
 ...
 *****/
bool DCPFA_controller::SetTargetPheromone() {
	argos::CVector2 pheromoneTarget;
	std::vector<argos::CVector2> pheromoneTrail;

	if(!PheromoneMemory.SelectTarget(RNG, pheromoneTarget, &pheromoneTrail)) {
		return false;
	}

	SetIsHeadingToNest(false);
	SetTarget(pheromoneTarget);
	TrailToFollow = pheromoneTrail;
	return true;
}
 ...
 *****/
argos::Real DCPFA_controller::GetExponentialDecay(argos::Real w, argos::Real time, argos::Real lambda) {
 ...
 *****/
argos::Real DCPFA_controller::GetBound(argos::Real value, argos::Real min, argos::Real max) {
	/* Calculate an offset. */
...
size_t DCPFA_controller::GetSearchingTime(){//qilu 10/22
    return searchingTime;
}
size_t DCPFA_controller::GetTravelingTime(){//qilu 10/22
    return travelingTime;
}

string DCPFA_controller::GetStatus(){//qilu 10/22
    //DEPARTING, SEARCHING, RETURNING
    if (DCPFA_state == DEPARTING) return "DEPARTING";
    else if (DCPFA_state ==SEARCHING)return "SEARCHING";
    else if (DCPFA_state == RETURNING)return "RETURNING";
    else if (DCPFA_state == SURVEYING) return "SURVEYING";
 ...
 *****/
argos::Real DCPFA_controller::GetPoissonCDF(argos::Real k, argos::Real lambda) {
...
void DCPFA_controller::UpdateTargetRayList() {
...
REGISTER_CONTROLLER(DCPFA_controller, "DCPFA_controller")
