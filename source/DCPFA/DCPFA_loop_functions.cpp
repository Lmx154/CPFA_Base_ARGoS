...
DCPFA_loop_functions::DCPFA_loop_functions() :
	...
	CommunicationRadiusSquared(4.0),
...
void DCPFA_loop_functions::Init(argos::TConfigurationNode &node) {	
 
	argos::CDegrees USV_InDegrees;
	argos::TConfigurationNode DCPFA_node = argos::GetNode(node, "DCPFA");

	argos::GetNodeAttribute(DCPFA_node, "ProbabilityOfSwitchingToSearching", ProbabilityOfSwitchingToSearching);
	argos::GetNodeAttribute(DCPFA_node, "ProbabilityOfReturningToNest",      ProbabilityOfReturningToNest);
	argos::GetNodeAttribute(DCPFA_node, "UninformedSearchVariation",         USV_InDegrees);
	argos::GetNodeAttribute(DCPFA_node, "RateOfInformedSearchDecay",         RateOfInformedSearchDecay);
	argos::GetNodeAttribute(DCPFA_node, "RateOfSiteFidelity",                RateOfSiteFidelity);
	argos::GetNodeAttribute(DCPFA_node, "RateOfLayingPheromone",             RateOfLayingPheromone);
	argos::GetNodeAttribute(DCPFA_node, "RateOfPheromoneDecay",              RateOfPheromoneDecay);
	argos::GetNodeAttribute(DCPFA_node, "PrintFinalScore",                   PrintFinalScore);

	if(argos::NodeExists(node, "communication")) {
		argos::TConfigurationNode communication_node = argos::GetNode(node, "communication");
		argos::GetNodeAttributeOrDefault(communication_node, "EnableDecentralizedSharing", EnableDecentralizedSharing, EnableDecentralizedSharing);
		argos::GetNodeAttributeOrDefault(communication_node, "CommunicationRadius", CommunicationRadius, CommunicationRadius);
    ...
    argos::LOG<<"Number of robots="<<Num_robots<<endl;
	   for(it = footbots.begin(); it != footbots.end(); it++) {
		      ...
		      DCPFA_controller& c2 = dynamic_cast<DCPFA_controller&>(c);
        c2.SetLoopFunctions(this);
        c2.SetPheromoneMemoryLimit(MaxPheromonesPerRobot);
	    }
...
void DCPFA_loop_functions::Reset() {
...
void DCPFA_loop_functions::PreStep() {
    SimTime++;
    ...
    if(curr_time_in_minutes - last_time_in_minutes==1){
	   ...
	   UpdatePheromoneList();
	   DecentralizedCommunicationStep();
...
void DCPFA_loop_functions::PostStep() {
	// nothing... yet...
}

bool DCPFA_loop_functions::IsExperimentFinished() {
...
void DCPFA_loop_functions::PostExperiment() {
    ...
    if (PrintFinalScore == 1) {
        ...
        string header = "./results/"+ type+"_DCPFA_r"+num_robots.str()+"_tag"+num_tag.str()+"_"+arena_width.str()+"by"+arena_width.str()+"_quard_arena_" + quardArena.str() +"_";
...
argos::CColor DCPFA_loop_functions::GetFloorColor(const argos::CVector2 &c_pos_on_floor) {
	return argos::CColor::WHITE;
}

void DCPFA_loop_functions::UpdatePheromoneList() {
...
void DCPFA_loop_functions::DecentralizedCommunicationStep() {
	if(EnableDecentralizedSharing == 0) {
		return;
	}

	const size_t ticks_per_second = GetSimulator().GetPhysicsEngine("dyn2d").GetInverseSimulationClockTick();
	size_t communication_period_ticks = static_cast<size_t>(CommunicationPeriodSeconds * ticks_per_second);
	if(communication_period_ticks == 0) {
		communication_period_ticks = 1;
	}
	if(GetSpace().GetSimulationClock() % communication_period_ticks != 0) {
		return;
	}

	const argos::Real current_time = getSimTimeInSeconds();
	std::vector<DCPFA_controller*> controllers;

	argos::CSpace::TMapPerType& footbots = GetSpace().GetEntitiesByType("foot-bot");
	for(argos::CSpace::TMapPerType::iterator it = footbots.begin(); it != footbots.end(); it++) {
		argos::CFootBotEntity& footBot = *argos::any_cast<argos::CFootBotEntity*>(it->second);
		BaseController& c = dynamic_cast<BaseController&>(footBot.GetControllableEntity().GetController());
		DCPFA_controller& dcpfa = dynamic_cast<DCPFA_controller&>(c);
		dcpfa.DecayAndPrunePheromoneMemory(current_time);
		controllers.push_back(&dcpfa);
	}

	for(size_t i = 0; i < controllers.size(); ++i) {
		for(size_t j = i + 1; j < controllers.size(); ++j) {
			const argos::CVector2 offset = controllers[i]->GetPosition() - controllers[j]->GetPosition();
			if(offset.SquareLength() > CommunicationRadiusSquared) {
				continue;
			}

			std::vector<DecentralizedPheromone> i_messages = controllers[i]->ExportActivePheromones();
			std::vector<DecentralizedPheromone> j_messages = controllers[j]->ExportActivePheromones();

			for(size_t msg = 0; msg < i_messages.size(); ++msg) {
				i_messages[msg].hop_count++;
			}
...
void DCPFA_loop_functions::RandomFoodDistribution() {
	FoodList.clear();
        FoodColoringList.clear();
...
void DCPFA_loop_functions::ClusterFoodDistribution() {
        FoodList.clear();
...
void DCPFA_loop_functions::PowerLawFoodDistribution() {
 FoodList.clear();
    FoodColoringList.clear();
	argos::Real foodOffset     = 3.0 * FoodRadius;
	size_t      foodPlaced     = 0;
...
bool DCPFA_loop_functions::IsOutOfBounds(argos::CVector2 p, size_t length, size_t width) {
...
bool DCPFA_loop_functions::IsCollidingWithNest(argos::CVector2 p) {
...
bool DCPFA_loop_functions::IsCollidingWithFood(argos::CVector2 p) {
...
unsigned int DCPFA_loop_functions::getNumberOfRobots() {
	return GetSpace().GetEntitiesByType("foot-bot").size();
}

double DCPFA_loop_functions::getProbabilityOfSwitchingToSearching() {
	return ProbabilityOfSwitchingToSearching;
}

double DCPFA_loop_functions::getProbabilityOfReturningToNest() {
	return ProbabilityOfReturningToNest;
}

// Value in Radians
double DCPFA_loop_functions::getUninformedSearchVariation() {
	return UninformedSearchVariation.GetValue();
}

double DCPFA_loop_functions::getRateOfInformedSearchDecay() {
	return RateOfInformedSearchDecay;
}

double DCPFA_loop_functions::getRateOfSiteFidelity() {
	return RateOfSiteFidelity;
}

double DCPFA_loop_functions::getRateOfLayingPheromone() {
	return RateOfLayingPheromone;
}

double DCPFA_loop_functions::getRateOfPheromoneDecay() {
	return RateOfPheromoneDecay;
}

argos::Real DCPFA_loop_functions::getSimTimeInSeconds() {
...
void DCPFA_loop_functions::SetTrial(unsigned int v) {
}

void DCPFA_loop_functions::setScore(double s) {
	score = s;
...
double DCPFA_loop_functions::Score() {	
	return score;
}

void DCPFA_loop_functions::increaseNumDistributedFoodByOne(){
    NumDistributedFood++;
}

void DCPFA_loop_functions::ConfigureFromGenome(Real* g)
{
...
REGISTER_LOOP_FUNCTIONS(DCPFA_loop_functions, "DCPFA_loop_functions")
